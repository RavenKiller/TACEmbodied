...
class WaypointPredictionNet(Net):
    def __init__(self, observation_space: Space, model_config: Config) -> None:
        ...
        self.inst_attn_k = nn.Conv1d(self.instruction_encoder.output_size, dk_inst, 1)
    ...
    def distance_to_continuous(self, distance: Tensor) -> Tensor:
        ...
        meters_per_distance = range_dist / (self.wypt_cfg.discrete_distances - 1)
        return self.wypt_cfg.min_distance_prediction + (distance * meters_per_distance)
    ...
    def offset_to_continuous(self, offset: Tensor) -> Tensor:
        ...
        rad_per_offset = radians_per_pano / (self.wypt_cfg.discrete_offsets - 1)
    ...
    def _init_distance_linear(self, in_dim: int, final_feature_size: int) -> None:
        """Initialize the distance output to be either discrete or
        continuous. If continuous, the distribution is TruncatedNormal.
        """
        if self.wypt_cfg.continuous_distance:
            self.distance_linear = nn.Sequential(nn.Linear(in_dim, 1), nn.Sigmoid())
            self.distance_var_linear = nn.Sequential(
                nn.Linear(self._hidden_size + final_feature_size, 1),
                nn.Sigmoid(),
            )
        else:
            self.distance_linear = nn.Linear(in_dim, self.wypt_cfg.discrete_distances)

    def _init_offset_linear(self, in_dim: int, final_feature_size: int) -> None:
        ...
        if self.wypt_cfg.continuous_offset:
            ...
            self.offset_linear = nn.Linear(in_dim, self.wypt_cfg.discrete_offsets)
    ...
    def forward(
        ...
        masks: Tensor,
    ) -> Tuple[CustomFixedCategorical, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor,]:
        """
        Returns:
        ...
        rgb_obs = torch.cat(
            [
                observations["rgb"],
                (observations["rgb_history"].permute(1, 2, 3, 0) * masks.squeeze(1))
                .permute(3, 0, 1, 2)
                .unsqueeze(1),
            ],
            dim=1,
        ...
        depth_obs = torch.cat(
            [
                observations["depth"],
                (observations["depth_history"].permute(1, 2, 3, 0) * masks.squeeze(1))
                .permute(3, 0, 1, 2)
                .unsqueeze(1),
            ],
            dim=1,
        ...
        depth_obs = depth_obs.view((depth_size[0] * depth_size[1],) + depth_size[2:5])
        ...
        text_q_spatial = text_q_spatial.repeat_interleave(self._num_panos).view(
        ...
        depth_kv_in = self.depth_kv_spatial(flat_depth_embedding)  # [B*12, 384, 16]
