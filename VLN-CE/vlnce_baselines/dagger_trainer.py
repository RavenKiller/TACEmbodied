...
class DaggerTrainer(BaseVLNCETrainer):
    ...
    def _update_dataset(self, data_it):
        ...
        if not self.config.MODEL.DEPTH_ENCODER.trainable:
            ...
            depth_hook = (
                self.policy.net.depth_encoder.visual_encoder.register_forward_hook(
                    hook_builder(depth_features)
                )
            )
        new_depth_features = None
        ...
        with tqdm.tqdm(
            ...
            while collected_eps < self.config.IL.DAGGER.update_size:
                ...
                new_depth_features = (
                    self.policy.net.new_depth_encoder.get_new_depth_features()
                )
                ...
                for i in range(envs.num_envs):
                    ...
                    if new_depth_features is not None:
                        observations[i]["new_depth_features"] = new_depth_features[i]
