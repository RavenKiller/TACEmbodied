...
class MyAccessMgr(AgentAccessMgr):
    def __init__(
        self,
        config: "DictConfig",
    ...
    def _init_policy_and_updater(self, lr_schedule_fn, resume_state):
        ...
        if resume_state is not None:
            ...
            self._updater.optimizer.load_state_dict(resume_state["optim_state"])
    ...
    def _create_updater(self, actor_critic) -> PPO:
        ...
        logger.info(
            "trainable number of parameters: {}".format(
                sum(
                    param.numel()
    ...
    def _create_policy(self) -> NetPolicy:
        ...
        if self._config.habitat_baselines.rl.policy.depth.pretrained and self._config.habitat_baselines.rl.policy.name != 'HierarchicalPolicy':
            ...
            actor_critic.net.depth_encoder.model.load_state_dict(
                {
                    k[len(prefix) :]: v
