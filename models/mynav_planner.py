...
class MyVqaLstmCnnAttentionModel(nn.Module):
    def __init__(
        self,
        q_vocab: Dict,
        ans_vocab: Dict,
        ...
        super(MyVqaLstmCnnAttentionModel, self).__init__()
        ...
        self.cnn_fc_layer = nn.Sequential(
            nn.Linear(32 * 12 * 12 + 128, 64), nn.ReLU(), nn.Dropout(p=0.5)
        )
        # !! add depth encoder
        self.config = config
        if "depth" in config.habitat_baselines.il:
            ...
            # self.depth_fc_layer = nn.Identity()
    ...
    def forward(
        self, images: Tensor, depths: Tensor, questions: Tensor
    ) -> Tuple[Tensor, Tensor]:
        ...
        if (
            self.config.habitat_baselines.il.depth.is_blind
        ):  # for baseline, zero the depth features
            depth_feats = torch.zeros((img_feats.shape[0], 128)).to(img_feats.device)
        else:
            depth_feats = self.depth_encoder.forward_eqa(
                depths.contiguous().view(
                    -1, depths.size(2), depths.size(3), depths.size(4)
                )
            )
...
class MyNavPlannerControllerModel(nn.Module):
    def __init__(
        self,
        q_vocab: Dict,
        num_output: int = 4,
        ...
        super(MyNavPlannerControllerModel, self).__init__()

        self.cnn_fc_layer = nn.Sequential(
            nn.Linear(32 * 12 * 12 + 128, planner_rnn_image_feat_dim),
            nn.ReLU(),
            nn.Dropout(p=0.5),
        )
