...
class MyEQADataset(wds.Dataset):
    ...
    def __init__(
        ...
        with habitat.Env(config=self.config) as self.env:
            ...
            self.frame_dataset_path = (
                config.habitat_baselines.il.frame_dataset_path.format(split=self.mode)
            )
            ...
            self.only_vqa_task = config.habitat_baselines.il.only_vqa_task
    ...
    def group_by_keys_(
        ...
        for fname, value in data:
            ...
            if prefix is None:
                continue
            isdepth = ".depth" in prefix
            if isdepth:
                suffix = "depth." + suffix
            prefix = prefix.replace(".depth", "")
            if lcase:
                suffix = suffix.lower()
