...
from models.tac_policy import DepthEncoder
...
class MyNavDataset(wds.Dataset):
    ...
    def __init__(
        ...
        self.eval_save_results = config.habitat_baselines.il.eval_save_results
        ...
        self.frame_dataset_path = config.habitat_baselines.il.frame_dataset_path.format(
            split=self.mode
        )
        ...
        cnn_kwargs = {
            "only_encoder": True,
            "checkpoint_path": config.habitat_baselines.il.eqa_cnn_pretrain_ckpt_path,
        }
        self.cnn = MultitaskCNN(**cnn_kwargs)
        self.cnn.eval()
        self.cnn.to(self.device)
        # !! add depth encoder
        if "depth" in config.habitat_baselines.il:
            self.depth_config = config.habitat_baselines.il.depth
            self.depth_encoder = DepthEncoder(self.depth_config)

            if config.habitat_baselines.il.depth.pretrained:
                logger.info(
                    "loading pretrained depth encoder from {}".format(
                        config.habitat_baselines.il.depth.pretrained_weights
                    )
                )
                ...
                self.depth_encoder.model.load_state_dict(
                    {
                        k[len(prefix) :]: v
            ...
            self.depth_fc_layer = nn.Sequential(
                nn.Linear(768, 128), nn.ReLU(), nn.Dropout(p=0.4)
            )
            self.depth_encoder.to(self.device)
            self.depth_fc_layer.to(self.device)
        ...
        if not self.cache_exists():
            ...
            logger.info(
                "Number of {} episodes: {}".format(self.mode, len(self.episodes))
            )
            ctr = 0
            for scene in tqdm(
            ...
            logger.info("[ Deleting dataset folder. This will take a few minutes. ]")
    ...
    def get_img_features(
        self, img: np.ndarray, preprocess: bool = False, depth: np.array = None
    ) -> torch.Tensor:
        if preprocess:
            imgs = (
            ...
            depths = torch.from_numpy(depth.transpose(2, 0, 1)).float().repeat(3, 1, 1)
            depths = depths.view(1, *(depths.shape)).to(self.device)
        else:
            if depth is None:
                N = len(img)
                assert N % 2 == 0
        ...
        if (
            self.raw_config.habitat_baselines.il.depth.is_blind
        ):  # for baseline, zero the depth features
            depth_feats = torch.zeros((img_feats.shape[0], 128)).to(img_feats.device)
        else:
            depth_feats = self.depth_encoder.forward_eqa(depths)
            depth_feats = self.depth_fc_layer(depth_feats)
        return torch.cat((img_feats, depth_feats), dim=1)
    ...
    def get_hierarchical_features_till_spawn(
        ...
        raw_img_feats = (
            self.get_img_features(self.frame_queue, depth=self.depth_queue)
            .cpu()
            .detach()
            .numpy()
            .copy()
        ...
        target_pos_idx = min(target_pos_idx, len(raw_img_feats) - 1)
    ...
    def preprocess_actions(self) -> None:
        ...
        for ep in self.episodes:
            ...
            ep.planner_actions[: len(planner_actions)] = torch.Tensor(planner_actions)
            ...
            ep.planner_hidden_idx[: len(controller_actions)] = torch.Tensor(ph_idx)
    ...
    def group_by_keys_(
        ...
        for fname, value in data:
            ...
            if not current_sample or prefix != current_sample["__key__"]:
                ...
                current_sample["episode_id"] = self.episodes[episode_id].episode_id
    ...
    def map_dataset_sample(self, x: Dict) -> Tuple:
        ...
        img_depths = [
            (
                ...
                if img.max() > 1.01
                else img.transpose(2, 0, 1)
            )
            for img in list(x.values())[4:]
        ]
        mixed = True
        if img_depths[0].shape[2] != img_depths[-1].shape[2]:
            mixed = False
        ...
        if mixed:
            ...
            self.frame_queue = np.array(img_depths[: int(len(img_depths) // 2)])
            self.depth_queue = np.array(img_depths[int(len(img_depths) // 2) :])
            self.frame_queue = torch.Tensor(self.frame_queue).to(self.device)
            self.depth_queue = torch.Tensor(self.depth_queue).to(self.device)
        ...
        planner_img_feats[: self.episodes[idx].planner_action_length] = img_feats[
        ...
        for i in range(controller_action_length):
            if (
                ...
                and (
                    ...
                    or controller_out[i - self.max_controller_actions + 1 : i].sum()
                    == self.max_controller_actions - 1
                )
            ):
                controller_mask[i] = 0
