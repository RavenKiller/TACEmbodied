...
# @baseline_registry.register_trainer(name="myddppo")
@baseline_registry.register_trainer(name="rearrange")
class PPOTrainer(BaseRLTrainer):
    ...
    def _pause_envs(
        ...
        if len(envs_to_pause) > 0:
            ...
            self._agent._actor_critic._modules['_high_level_policy']._next_sol_idxs = self._agent._actor_critic._modules['_high_level_policy']._next_sol_idxs[state_index]
            self._agent._actor_critic._cur_skills = self._agent._actor_critic._cur_skills[state_index]
    ...
    def _training_log(self, writer, losses: Dict[str, float], prev_time: int = 0):
        ...
        if self.num_updates_done % self.config.habitat_baselines.log_interval == 0:
            logger.info(
                "update: {}\tfps: {:.3f}\tsteps: {}\t".format(
                    self.num_updates_done, fps, self.num_steps_done
                )
            )
    ...
    def train(self) -> None:
        ...
        # Set a timestamp subdir for different runs
        with read_write(self.config):
            self.config.habitat_baselines.tensorboard_dir = os.path.join(
    ...
    def eval(self) -> None:
        ...
        with read_write(self.config):
            self.config.habitat_baselines.tensorboard_dir = os.path.join(
                self.config.habitat_baselines.tensorboard_dir,
                self.config.habitat_baselines.rl.policy.name
                + datetime.now().strftime("_%Y%m%d-%H%M%S_EVAL"),
            )
    ...
    def _eval_checkpoint(  # noqa: C901
        ...
        #     observations = assign_new_depth(observations)
        batch = batch_obs(observations, device=self.device)
        batch = apply_obs_transforms_batch(batch, self.obs_transforms)  # type: ignore
        ...
        self._agent.eval()
        if hasattr(self._agent.actor_critic, "_skills"):
            for k in self._agent.actor_critic._skills:
                if hasattr(self._agent.actor_critic._skills[k], "_wrap_policy"):
                    self._agent.actor_critic._skills[k]._wrap_policy.eval()
        successes = []
        successesh = []
        while (
