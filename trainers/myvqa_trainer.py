...
from datasets.myeqa_dataset import MyEQADataset
from models.mynav_planner import MyVqaLstmCnnAttentionModel
...
@baseline_registry.register_trainer(name="myvqa")
class VQATrainer(BaseILTrainer):
    ...
    def train(self) -> None:
        ...
        if "depth" in config.habitat_baselines.il:
            vqa_dataset = (
                ...
                .to_tuple(
                    ...
                    *["depth.{0:0=3d}.jpg".format(x) for x in range(0, 5)],
                )
        ...
        model = MyVqaLstmCnnAttentionModel(**model_kwargs)
        ...
        with TensorboardWriter(
            ...
            while epoch <= config.habitat_baselines.il.vqa.max_epochs:
                ...
                for batch in train_loader:
                    t += 1
                    _, questions, answers, frame_queue, depth_queue = batch
                    optim.zero_grad()

                    questions = questions.to(self.device)
                    answers = answers.to(self.device)
                    frame_queue = frame_queue.to(self.device)
                    depth_queue = depth_queue.to(self.device)

                    scores, _ = model(frame_queue, depth_queue, questions)
                ...
                logger.info("Average accuracy: {:.3f}".format(avg_accuracy))
                logger.info("Average mean rank: {:.3f}".format(avg_mean_rank))
                logger.info(
                    "Average mean reciprocal rank: {:.3f}".format(
    ...
    def _eval_checkpoint(
        ...
        with torch.no_grad():
            for batch in eval_loader:
                t += 1
                episode_ids, questions, answers, frame_queue, depth_queue = batch
                questions = questions.to(self.device)
                answers = answers.to(self.device)
                frame_queue = frame_queue.to(self.device)
                depth_queue = depth_queue.to(self.device)
        ...
        logger.info("Average mean rank: {:.3f}".format(avg_mean_rank))
        logger.info(
            "Average mean reciprocal rank: {:.3f}".format(avg_mean_reciprocal_rank)
        )
