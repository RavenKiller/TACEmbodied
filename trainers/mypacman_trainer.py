...
from models.mynav_planner import MyNavPlannerControllerModel
from habitat_baselines.utils.common import generate_video
from datasets.mynav_dataset import MyNavDataset

cv2 = try_cv2_import()


@baseline_registry.register_trainer(name="mypacman")
class PACMANTrainer(BaseILTrainer):
    ...
    def train(self) -> None:
        ...
        with habitat.Env(config.habitat) as env:
            ...
            if "depth" in self.config.habitat_baselines.il:
                model = MyNavPlannerControllerModel(**model_kwargs)
            else:
    ...
    def _eval_checkpoint(  # noqa: C901
        ...
        with habitat.Env(config.habitat) as env, torch.no_grad():
            if "depth" in config.habitat_baselines.il:
                nav_dataset = MyNavDataset(
                    config,
                    env,
                    self.device,
                ).decode("rgb")
            ...
            nav_dataset.depth_encoder.eval()
            nav_dataset.depth_fc_layer.eval()
            ...
            for t, batch in enumerate(eval_loader):
                ...
                for i in [10, 30, 50, "rand_init"]:
                    for j in ["pred", "fwd-only"]:
                        ...
                        # compute success rate within a distance
                        for threshold in [0.5, 1.0, 1.5, 2.0, 3.0]:
                            metrics_slug["sr_{}{}_{}".format(threshold, m, i)] = float(
                                dists_to_target[-1] <= threshold
                            )
